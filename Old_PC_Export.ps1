# Old_PC_Export.ps1
# 14-7-2022: v1.6 Changed commands for Firefox copy/restore, should make firefox transfers more robust
# 6-7-2022:  v1.5 Removed the copying of the taskbar icons, while it worked often, sometimes it was more of a hinderance than a help and I want the script to be consistent
# 12-6-2022: v1.4 Reduced retry limit and wait time to 2 seconds each to prevent being caught in non-stop loop
# 25-5-2022: v1.3 Moved log locations from root to Script logs folder. Added MT flag to robocopy to optimise speed in some scenarios
# 5-5-2022:  v1.2 Added special location of downloads folder rather than assumed static location. Added test for special folders on network drive and prompt whether or not to copy from there.
# 17-4-2022: v1.1 Changed from Copy-Item to robocopy for the majority of file transfers, adding compatibility for all files in paths longer than 256 characters. Changed from Get-Volume to Get-PSDrive for better compatibility with Windows 11.
# 12-4-2022: v1.0 Created a migration script for Windows 10 computers, copying all files onto a external drive
